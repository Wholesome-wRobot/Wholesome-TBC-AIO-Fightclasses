...
namespace WholesomeTBCAIO.Rotations.Priest
{
    public class HolyPriestParty : Priest
    {
        ...
        protected override void HealerCombat()
        {
            ...
            // PARTY Shield
            if (PowerWordShield.IsSpellUsable)
            {
                List<AIOPartyMember> needShield = aliveMembers
                    .FindAll(m => m.HealthPercent < 40 && !m.HaveBuff("Power Word: Shield") && !ToolBox.HasDebuff("Weakened Soul", m.Name))
                    .OrderBy(m => m.HealthPercent)
                    .ToList();
                if (needShield.Count > 0 && cast.OnFocusUnit(PowerWordShield, needShield[0]))
                    return;
            }
            ...
            // PARTY Flash heal
            List<AIOPartyMember> needFlashHeal = aliveMembers
            ...
            if (PrayerOfMending.IsSpellUsable)
            {
                List<AIOPartyMember> needPrayerOfMending = aliveMembers
                    .OrderBy(m => m.HealthPercent)
                    .ToList();
            ...
            // PARTY Renew Rank 10
            List<AIOPartyMember> needRenew10 = aliveMembers
            ...
            List<AIOPartyMember> needPrayerOfHealing = aliveMembers
            ...
            {
                List<AIOPartyMember> needGHeal2 = aliveMembers
            ...
            {
                List<AIOPartyMember> needHeal70 = aliveMembers
                    .FindAll(m => m.HealthPercent < 80)
                    .OrderBy(m => m.HealthPercent)
                    .ToList();
